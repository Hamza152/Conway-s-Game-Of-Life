...
bool GifBegin( GifWriter* writer, const wchar_t* filename, uint32_t width, uint32_t height, uint32_t delay, int32_t bitDepth = 8, bool dither = false )
{
    (void)bitDepth; (void)dither; // Mute "Unused argument" warnings
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	writer->f = 0;
    _wfopen_s(&writer->f, filename, L"wb");
#else
    writer->f = wfopen(filename, L"wb");
#endif
